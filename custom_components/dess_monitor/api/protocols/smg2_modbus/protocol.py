...
Transport-agnostic: we hand opaque RTU bytes to whatever
:class:`DirectTransport` is paired with us. In this integration that's
``CloudHexTransport`` — the DESS cloud relays raw bytes to the device's
serial line, where SMG-II firmware parses them as Modbus.

Register layout cribbed from ``dess-monitor-local``.
"""
